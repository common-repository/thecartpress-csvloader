...
if ( isset( $_REQUEST['tcp_load_csv'] ) && isset( $_FILES['upload_file'] ) && strlen( $_FILES['upload_file']['tmp_name'] ) > 0 ) {
	...
	if ( is_array( $data ) ) {
		...
		foreach( $data as $cols ) {
			...
			//If SKU and exists a product, in the database, with the same SKU then Update the exinting product 
			if ( strlen( $sku ) > 0 ) {
				...
				if ( $post_id ) {
					$post['ID']				= $post_id; //Update a previous product
					$current_post			= get_post( $post_id );//Retrieves all the info of the current post
			...
			if ( $process_this_row ) {
				...
				//Here is Where the product (or any other post type) is created
				...
				//Fixed values (non supported by CSV Loader)
				...
				//Process attachments (more than one)
				//First, remove the current attachments
				...
				//Second, add the new attachmetns
				...
				//Allowing to add more fields by plugins
...
<form method="post" enctype="multipart/form-data">
	...
	<tr valign="top">
		<th scope="row">
			<label for="display_rows"><?php _e( 'Display Rows', 'tcp_csvl' ); ?>:</label>
		</th>
		<td>
	...
	<tr valign="top">
		<th scope="row">
			<label for="upload_file"><?php _e( 'file', 'tcp_csvl' ); ?>:</label>
		</th>
		<td>
...
<table class="widefat fixed">
	...
	<thead>
		<tr class="manage-column">
			<th>&nbsp;</th>
			<?php foreach( $titles as $col ) echo '<th scope="col">', $col, '</th>'; ?>
		</tr>
	</thead>
	<tfoot>
		<tr class="manage-column">
	...
	<tbody>
		<?php foreach( $data as $i => $cols ) :
			if ( $i > 4 && $display_rows == 'four' ) :
				...
				<tr>
					<td><?php echo $i; ?></td>
					<?php foreach( $cols as $col ) echo '<td>', $col, '</td>'; ?>
				</tr>
			<?php endif; ?>
		<?php endforeach; ?>
	</tbody>
...
<form method="post">
	...
	<thead>
		<tr class="manage-column">
			<th scope="col"><?php _e( 'Imported columns', 'tcp_csvl' ); ?></th>
			<th scope="col"><?php _e( 'TheCartPress columns', 'tcp_csvl' ); ?></th>
		</tr>
	</thead>
	<tfoot>
		<tr class="manage-column">
			<th scope="col"><?php _e( 'CSV columns', 'tcp_csvl' ); ?></th>
			<th scope="col"><?php _e( 'TheCartPress columns', 'tcp_csvl' ); ?></th>
		</tr>
	</tfoot>
	<tbody>
	<?php if ( is_array( $titles ) && count( $titles ) > 0 ) : ?>
	<?php //for each column in the CSV
	foreach( $titles as $i => $col ) : ?>
		<tr>
			<td><?php echo $col; ?></td>
			<td>
			<?php $upper_col = strtoupper( $col );
			//$options = array( { id, selected(true/false), title }, );
			$options = array();
			...
			//Adding Custom Fields to options list
			...
			//Adding Taxonomies to options list
			...
			//Allows to add more fields to be assigned
			//$options = array( { id, selected(true/false), title }, ); 
			$options = apply_filters( 'tcp_csvl_option_columns', $options, $col ); ?>
				<select name="col_<?php echo $i; ?>[]" multiple size="6" style="height: auto;">
	...
	<p>
		<label for="tcp_cat"><?php _e( 'Attach products to the Taxonomy', 'tcp_csvl' )?>:</label>
